...
# from toch.utils.data.distributed import DistributedSampler
...
# torch.cuda.set_device(0,1,8,9)
os.environ["CUDA_VISIBLE_DEVICES"] = '0,1'

def train_model(model, train_dataloader, loss_fn, optimizer, epoch, use_gpu,start,model_type,device):
    # model.train()
    ...
    for idx, (inputs, labels) in enumerate(train_dataloader):
        ...
        #     # labels = labels.to('cuda') # to modify or not 
        ...
        total_corrects += torch.sum(preds.eq(labels.to(device)))
...
def main(model_type, num_epochs, lr, momentum,device,local_rank):
    ...
    distrib_model = torch.nn.parallel.DistributedDataParallel(model,device_ids=[local_rank],output_device=local_rank)
    # lr = lr
    # momentum = momentum
    ...
    train_dataloader = tud.DataLoader(train_imgs, batch_size=batch_size,sampler=train_sampler,num_workers = 16, pin_memory = True)
    ...
    test_dataloader = tud.DataLoader(test_imgs, batch_size=batch_size,num_workers = 16,pin_memory = True)
    ...
    for epoch in range(num_epochs):
        ...
        train_model(distrib_model, train_dataloader, loss_fn, optimizer, epoch, use_gpu,start,model_type,device)
...
# local_rank is the label of local GPU
parser = argparse.ArgumentParser()
parser.add_argument('--local_rank',default=-1,type=int,help='node rank for distributed training')
args = parser.parse_args()
print('Here is all local GPUs: ',args.local_rank)
...
for i in range(len(model_types)):
    ...
    main(model_types[i], num_epochs, lr, momentum,device,args.local_rank)
